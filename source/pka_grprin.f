      subroutine pka_grprin(infile,irxn, select_energy)
c***********************************************************************
...
c
      if ( icon(9) .lt. 1) then 
         write (6,7823)
 7823    format (1x, '*** Enter PKA_GRPRIN ')
      endif
      ename = 'opt'
      lename = lnblnk(ename)
...
c        read(7,9110) ilow, ihigh, index,irxn2
 ...
 1971      format (1x, 'echo-2: ', 5i5, g14.7)
        endif
        if ( ilow .gt. 1) then 
           do jk=1,ilow-1
...
c
 ...
 924         format (1x, 'renorm: ', 3i5, g14.7)
         endif
         sum = 0.0
...
c
 ...
 5613 format (/,/, 5x, ' Robinson Damge energy computed using: ',/,
     &             5x, ' File                           = ', a,/,
     &             5x, ' Reaction                       = ', i3,/,
...
c            DE = 1.0
         ...
         enddo
c         if ( sum .le. 0.0) sum = 1.e-35
         robinson_DE(ie) = sum
      enddo
c
c     Compute the dE/dE PKA spectrum
c
      do ie = 1, nenergy
         do je = ipka_low_energy(ie), ipka_high_energy(ie)
...
c      enddo
c
c     Write a number fraction histogram representation of the PKA spectra
c
      iblank = lnblnk(infile)
      leopt = lnblnk(optical_old)
...
c
 ...
 914         format (1x, 'rehash: ', 3i5, g14.7)
     ...
     &       .and. sintg .gt. 0.0 ) then 
            open(unit=47,file=temp_file_out(1:iend1)//'_FF_'
     &      //reaction_label(1:6)
     &          //'_'//energy_label(1:15), status='unknown')
            write (47, 815) energy(ipka_low_energy(ie)), 
     &                   pka_spectrum(ie,ipka_low_energy(ie))*1.e-6
            do je = ipka_low_energy(ie), ipka_high_energy(ie)
               write (47, 815) energy(je),   pka_spectrum(ie,je)
               write (47, 815) energy(je+1), pka_spectrum(ie,je)
 815           format (1x, 2g15.7)
            enddo
            write (47, 815) energy(ipka_high_energy(ie)+1), 
     &                   pka_spectrum(ie,ipka_high_energy(ie))*1.e-6
            close (unit=47)
         endif
      enddo
c
...
2814     format (1x, 'PKA dEdE spectrum write: ', a)
     ...
     &       (ipka_high_energy(ie) .ge. ipka_low_energy(ie))
     &        .and. sintg .gt.0.0 ) then 
            open(unit=47,file=temp_file_out(1:iend1)//'_dede_'
     &      //reaction_label(1:6)
     &          //'_'//energy_label(1:15), status='unknown')
            write (47, 2815) energy(ipka_low_energy(ie)), 
     &                   pka_spectrum_dede(ie,ipka_low_energy(ie))
     &                   *1.e-6
            do je = ipka_low_energy(ie), ipka_high_energy(ie)
               write (47, 2815) energy(je),   pka_spectrum_dede(ie,je)
               write (47, 2815) energy(je+1), pka_spectrum_dede(ie,je)
2815           format (1x, 2g15.7)
            enddo
            write (47, 2815) energy(ipka_high_energy(ie)+1), 
     &                   pka_spectrum_dede(ie,ipka_high_energy(ie))
     &                   *1.e-6
            close (unit=47)
         endif
c
...
c
      open(unit=57,file=temp_file_out(1:iend1)//'_'//
     &          reaction_label(1:6)//'.DE',
     &    status='unknown')
      write (57, 451) (emid(ie), robinson_DE(ie),ie=1,nenergy)
 451  format (1x, 2g15.7)
      close (unit=57)
c
...
c
 ...
 7812               format (1x,'Recoil spectrum for neutron energy = ',
     ...
     &                   'Fraction',4x,'     ')
                    ...
                    do je = ipka_low_energy(ie), ipka_high_energy(ie)
                       icount = icount + 1
                       write (6,7813) icount, energy(je)*1.e-6, 
     &                    energy(je+1)*1.e-6, 
     &                    pka_spectrum(ie,je), pka_spectrum_dede(ie,je)
                    enddo
 7813               format (3x,i5,3x,1pg14.7,3x,1pg14.7,2x,g14.7, 
     &                  2x, g14.7, 2x)
                    write (6,491) robinson_DE(ie)*1.e-6, 
     &                    robinson_DE(ie)/emid(ie)*100.
...
c
c     Find the energy bin for the selected_energy point
c
      iselect = 76
      do ie = 1,nenergy
        if ( energy(ie)*1.e-6   .le. select_energy .and. 
     &       energy(ie+1)*1.e-6 .gt. select_energy) then 
            iselect = ie
        endif
      enddo
c
      if ( nenergy .eq. 81) then 
          do ie = 1, nenergy
             if ( ie .eq. iselect) then 
                if ((ipka_low_energy(ie)  .gt. 0) .and. 
     &              (ipka_high_energy(ie) .ge. 
     &               ipka_low_energy(ie))) then 
                    write (6, 9812) emid(ie)*1.e-6
 9812               format (1x,'Recoil spectrum for selected',
     &              ' neutron energy = ',
     &              g14.7, ' MeV',/,/,6x,
     &              'Bin', 3x, '  Lower PKA   ',4x, ' Upper PKA  ',4x,
     &                   ' Number ',9x,'dE/dE',/,6x,
     ...
     &                    energy(je+1)*1.e-6, 
     &                    pka_spectrum(ie,je), pka_spectrum_dede(ie,je)
                       write (39, 6712) je, energy(je)*1.e-6, 
     &                    energy(je+1)*1.e-6, pka_spectrum_dede(ie,je)
 6712                  format (1x, i5, 2x, 3g14.7)
                    enddo
c 7813               format (3x,i5,3x,1pg14.7,3x,1pg14.7,2x,g14.7, 
c     &                  2x, g14.7, 2x)
                    write (6,491) robinson_DE(ie)*1.e-6, 
     &                    robinson_DE(ie)/emid(ie)*100.
c 491                format (/, 4x, 'Robinson Damage Energy = ', 1pg14.7,
c     &              '  MeV',/, 4x, 'Damage Energy Percent  = ', 2pg12.4,
c     &               ' %',/)
                endif
             endif
          enddo
